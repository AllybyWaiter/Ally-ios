...
import { Plus, MoreVertical, Pencil, Trash2, Fish, Bug, Flower2, HelpCircle, Activity } from 'lucide-react';
...
export function AquariumLivestock({ aquariumId }: AquariumLivestockProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast({ title: 'Success', description: 'Livestock removed successfully' });
      await loadLivestock();
    } catch (error: any) {
      console.error('Error deleting livestock:', error);
      toast({
        title: 'Error',
        description: 'Failed to remove livestock',
        variant: 'destructive',
      });
    } finally {
